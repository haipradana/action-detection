#!/usr/bin/env python3
"""
Action Recognition with MERL Clips Data
...
print("🎯 ACTION RECOGNITION WITH MERL CLIPS DATA")
print("=" * 60)
...
# Data paths
clips_base_path = '../merl-shopping/clips'
...
if not os.path.exists(clips_base_path):
    ...
    print("   Please run merl-shopping det2rec.py first to generate clips")
    exit(1)

if not os.path.exists(dataframes_path):
    print(f"❌ Error: Dataframes path not found: {dataframes_path}")
    print("   Please run merl-shopping det2rec.py first to generate dataframes")
    exit(1)
...
def train_model():
    ...
    for epoch in range(CONFIG['epochs']):
        ...
        try:
            for batch_idx in range(min(len(train_data), 50)):  # Limit batches for demo
                ...
                if batch_idx % 10 == 0:
                    print(f"   📊 Batch {batch_idx}/{min(len(train_data), 50)} - "
            ...
            for batch_idx in range(min(len(val_data), 20)):  # Limit val batches
