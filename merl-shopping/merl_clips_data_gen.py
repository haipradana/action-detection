#!/usr/bin/env python3
"""
MERL Clips Data Generator (Consolidated Version)
...
class MerlClipsDataGenerator(Sequence):
    ...
    def __init__(self, clips_base_path="clips", dataframes_path="dataframes", 
                 seq_len=15, batch_size=4, target_size=(224, 224), 
                 shuffle=True, augment=False):
        """
        Args:
            clips_base_path: Path to clips folder (default: 'clips')
            dataframes_path: Path to dataframes folder (default: 'dataframes')
    ...
    def _load_all_clips(self):
        ...
        if not dataframe_files:
            print(f"⚠️ No dataframe files found in {self.dataframes_path}")
            print("Make sure to run det2rec.py first to generate clips")
            return []
        ...
        for df_file in dataframe_files:
            ...
            try:
                ...
                print(f"⚠️ Could not parse video number from {df_file}")
                continue
        ...
        print(f"📁 Loaded {len(clip_data)} clips from {len(dataframe_files)} dataframes")
        return clip_data
    
    def _print_class_distribution(self):
        """Print class distribution statistics"""
        if not self.clip_data:
            print("❌ No clip data available")
            return
    ...
    def __getitem__(self, index):
        if not self.clip_data:
            # Return dummy data if no clips available
            batch_x = np.zeros((1, self.seq_len, *self.target_size, 3))
            batch_y = np.array([4])  # Default to "Inspect Shelf"
    ...
    def _preprocess_clip(self, clip_frames):
        ...
        for frame in clip_frames:
            # Frame should already be in RGB and normalized from det2rec.py
            if frame.max() <= 1.0:
...
def create_merl_data_generators(clips_base_path="clips", dataframes_path="dataframes", 
...
if __name__ == "__main__":
    ...
    if len(test_gen) > 0:
        ...
        print("💡 Run 'python utils/det2rec.py --start 1 --end 10' to generate some clips first") 